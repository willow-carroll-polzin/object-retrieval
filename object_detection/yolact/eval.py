    ...
    if args.display_text or args.display_bboxes:
        ...
        for j in reversed(range(num_dets_to_consider)):
            ...
            if args.display_text:
                ...
                #####
                #print(text_str + " " + str(classes[j]))
                #####
                obj_vector[classes[j]]=obj_vector[j]+1
                font_face = cv2.FONT_HERSHEY_DUPLEX
                font_scale = 0.6
                font_thickness = 1
...
def evaluate(net:Yolact, dataset, train_mode=False):
    ...
    if args.image is not None:
        ...
        obj_tensors=evalimages(net, inp, out)
        return obj_tensors
    ...
    try:
        ...
        for it, image_idx in enumerate(dataset_indices):
            ...
            if args.display:
                img_numpy,obj_tensor = prep_display(preds, img, h, w)
            elif args.benchmark:
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        obj_tensors=evaluate(net, dataset)
        torch.save(obj_tensors, "tensors.pckl")
        #obj_pickle = open("object_pickle.pckl","w")
        #pickle.dump(obj_tensors,obj_pickle)
        #obj_pickle.close()
